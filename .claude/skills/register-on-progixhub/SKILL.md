...
- **Env (optional — ASK FIRST) →** if a local `.env` / `.env.local` exists, ask the user whether to upload it. Values are **encrypted at rest** on progixHub (AES-256-GCM keyring); `NEXT_PUBLIC_*` keys are auto-scoped public. If yes, read the file and call **`upload_env`** with `{ projectId, dotenv }`. **Never upload env without explicit confirmation** — it contains secrets.

### 5 · Verify + hand off

- Call **`get_project_status`** and report the checklist (specs, docs, env, setup, portal).
- Tell the user the project is registered at `https://hub.progix.pro/projects/<id>`, with N specs + the product doc synced.
- Remind them the same MCP now powers day-to-day jobs from this repo:
  ...
  - **`upload_env` / `add_document` / `sync_specs`** — keep config, docs, and specs in sync as the project evolves.
